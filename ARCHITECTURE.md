...
`Pane:display` shows the log the moment the run starts, with the cursor on the last line, and rereads it every 500ms. When the run finishes it stops the timer and either shows the results file, when the status is `ok` and there is one, or rereads the log once more so the footer is on screen. `Source:output(view)` is what `:DBOutput` calls, and it reopens the window if it was closed. `"toggle"` picks whichever of the two files is not showing.
